...
return {
	...
	on_attach = function(client, bufnr)
		--- keybinds
		--- pin `main.typ` when working on multi-file projects
		--- see https://myriad-dreamin.github.io/tinymist/frontend/neovim.html#label-Working%20with%20Multiple-Files%20Projects
		vim.keymap.set("n", "<leader>wtp", function()
			client:exec_cmd({
				title = "pin",
				command = "tinymist.pinMain",
				arguments = { vim.api.nvim_buf_get_name(0) },
			}, { bufnr = bufnr })
			vim.notify("Pinned current file as main.", vim.log.levels.INFO)
		end, { desc = "[T]inymist [P]in", noremap = true, buffer = bufnr })

		vim.keymap.set("n", "<leader>wtu", function()
			client:exec_cmd({
				title = "unpin",
				command = "tinymist.pinMain",
